...
.status-bar {
    ...
    border-radius: 10px;
    padding: .6rem 1rem;
    font-size: .8rem; color: #66bb6a;
    margin-bottom: 1.2rem;
    animation: fadeUp 0.6s .15s ease both;
}
.status-dot {
...
.login-title {
    ...
    color: #e8e0d5; letter-spacing: 1px;
    margin: 0 0 1.4rem;
}
...
.ph-name {
    ...
    color: #FF8F00; letter-spacing: 1px;
    margin: 0;
}
...
.ph-badge {
    ...
    letter-spacing: .8px; padding: 2px 10px;
...
.ph-avatar {
    ...
    font-family: 'Rajdhani', sans-serif;
    font-weight: 700; font-size: 1.05rem; color: #FF8F00;
    flex-shrink: 0;
}
...
.info-card::after {
    ...
    background: linear-gradient(180deg, #FF6D00, #FF8F00);
    border-radius: 3px 0 0 3px;
}
.ic-label { font-size: .68rem; color: #444; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 4px; }
.ic-value {
    font-family: 'Rajdhani', sans-serif;
    font-size: 1.65rem; font-weight: 700; color: #FF8F00; line-height: 1;
}
...
.badge-warn   { background:rgba(230,81,0,0.1);   color:#FF8F00; border:1px solid rgba(255,109,0,0.2); padding:3px 11px; border-radius:20px; font-size:.76rem; font-weight:600; }
.badge-danger { background:rgba(183,28,28,0.1);  color:#ef9a9a; border:1px solid rgba(229,57,53,0.2); padding:3px 11px; border-radius:20px; font-size:.76rem; font-weight:600; }
...
.bloqueio-title {
    ...
    color:#ef5350; letter-spacing:3px;
}
...
.wpp-float a {
    ...
    padding: 11px 20px; font-weight: 600; font-size: .86rem;
    text-decoration: none !important;
    display: flex; align-items: center; gap: 8px;
    box-shadow: 0 4px 20px rgba(37,211,102,0.35);
    letter-spacing: .3px; transition: transform .2s;
}
...
.stButton > button:hover {
    ...
    box-shadow: 0 6px 22px rgba(230,81,0,0.5) !important;
}
...
div[data-baseweb="tab"][aria-selected="true"] {
    background: #1a0e00 !important; color: #FF8F00 !important;
}
...
div[data-baseweb="textarea"]:focus-within {
    ...
    box-shadow: 0 0 0 2px rgba(255,109,0,0.12) !important;
}

div[data-baseweb="select"] > div {
    background: #161616 !important; border-color: #222 !important;
    border-radius: 10px !important; color: #e8e0d5 !important;
}

div[data-testid="stExpander"] {
    background: #111 !important; border: 1px solid #1e1e1e !important;
    border-radius: 12px !important; margin-bottom: .4rem !important;
...
section[data-testid="stFileUploadDropzone"] {
    background: #111 !important; border: 1.5px dashed #222 !important;
...
FUNCIONARIOS = {
    ...
    "wagner":   {"nome":"Wagner",  "funcao":"Assistente Administrativo","senha":"wagner007","salario":650.0,"meta":100.0,"comissao_pct":0,"mostrar_salario":True, "mostrar_meta":False,"mostrar_comissao":False,"holerite_aviso":False,"is_admin":False},
...
def texto_horario(): return "Seg – Sex: 07:45 – 19:00  ·  Sáb: 07:45 – 13:00"

def fmt_moeda(v): return f"R$ {v:,.2f}".replace(",","X").replace(".",",").replace("X",".")

def wpp_link(msg): return f"https://wa.me/{WHATSAPP_NUMERO}?text={urllib.parse.quote(msg)}"
...
def tela_login():
    ...
    if st.button("ENTRAR  →", use_container_width=True):
        ...
        if u in FUNCIONARIOS:
            ...
            st.error("❌ Usuário não encontrado. Use: admin · sueli · leiliane · riquele · wagner · agnaldo · rogerio · samuel")
...
def tela_ponto(usuario, dados):
    ...
    if usuario not in dados["pontos"]:            dados["pontos"][usuario] = {}
    if hoje not in dados["pontos"][usuario]:       dados["pontos"][usuario][hoje] = {}
    ...
    if reg:
        ...
        for m in MARCACOES:
            h = reg.get(m,"–")
            st.markdown(f'<div class="ponto-row"><span class="ponto-label">{EMOJIS_PONTO[m]} &nbsp;{m}</span><span class="{"badge-ok" if h!="–" else "badge-warn"}">{h}</span></div>', unsafe_allow_html=True)
    ...
    if hist:
        ...
        for d in sorted(hist.keys(),reverse=True)[:10]:
            ...
            with st.expander(f"📅  {d}"):
                for m in MARCACOES:
                    h = r.get(m,"–")
                    st.markdown(f'<div class="ponto-row"><span class="ponto-label">{EMOJIS_PONTO[m]} &nbsp;{m}</span><span class="{"badge-ok" if h!="–" else "badge-danger"}">{h}</span></div>', unsafe_allow_html=True)
...
def tela_documentos(usuario, func, dados):
    ...
    if st.button("📤  Enviar Documento", use_container_width=True):
        if arq is None:
            ...
            if "documentos" not in dados: dados["documentos"] = []
            dados["documentos"].append({"usuario":usuario,"nome":func["nome"],"tipo":tipo,"obs":obs,"arquivo":arq.name,"data":data_str})
            salvar_dados(dados)
...
def painel_funcionario(usuario, func, dados):
    ...
    with tab2:
        ...
        if func["mostrar_comissao"] and func["comissao_pct"] > 0:
            st.markdown(f'<div class="info-card"><div class="ic-label">Comissão sobre vendas acima da meta</div><div class="ic-value">{func["comissao_pct"]}%</div></div>', unsafe_allow_html=True)
            if func["meta"] > 0:
...
def painel_admin(dados):
    ...
    with tab1:
        st.markdown('<div class="sec-title">Livro Ponto — Todos</div>', unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        <div style="padding:.4rem 0 .2rem;">
            <div style="width:42px;height:42px;border-radius:50%;background:rgba(255,109,0,0.1);border:1.5px solid rgba(255,109,0,0.28);display:flex;align-items:center;justify-content:center;font-family:'Rajdhani',sans-serif;font-weight:700;font-size:1rem;color:#FF8F00;margin-bottom:.5rem;">{av}</div>
            <div style="font-family:'Rajdhani',sans-serif;font-size:1.05rem;font-weight:600;color:#FF8F00 !important;letter-spacing:.8px;">{func['nome'].split()[0]}</div>
    ...
    render_wpp_float()
    painel_admin(dados) if func["is_admin"] else painel_funcionario(usuario, func, dados)
